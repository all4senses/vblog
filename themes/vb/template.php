 ...
 */
/*
function vb_preprocess_user_profile(&$profile) {
  //dpm($profile);
}
*/
 ...
 */
function vb_link($variables) {
  
  if (strpos($variables['path'], '://') && !strpos($variables['path'], 'ttp://www.voip-weblog.com')) {
 ...
 */
function vb_preprocess_user_picture(&$picture) {
  ...
  if (!empty($picture['account']->realname)) {
    ...
    $userExtendedData = vb_misc_loadUserExtendedData($picture['account']->uid);
 ...
 * @ingroup themeable
 */
function vb_pager_next($variables) {
  ...
  if ($out) {
    ...
    if(preg_match('|.*href="([^"]*)" .*title="([^"]*)" .*|', $out_altered, $matches) && !empty($matches[1])) {
      ...
      if ($matches[2] == 'Go to next page') {
        vb_misc_addMetatag('next', NULL, $href = 'http://www.voip-weblog.com' . $matches[1]);
      }
    }
  }
  return $out;
 ...
 * @ingroup themeable
 */
function vb_pager_previous($variables) {
  ...
  if ($out) {
    ...
    if(preg_match('|.*href="([^"]*)" .*title="([^"]*)" .*|', $out_altered, $matches) && !empty($matches[1])) {
      ...
      if ($matches[2] == 'Go to previous page') {
        vb_misc_addMetatag('prev', NULL, $href = 'http://www.voip-weblog.com' . $matches[1]);
      }
...
//  }
//  elseif ( $arg_0 == 'node' && (in_array($arg_1, $altered_pager_reviews) || vb_misc_identifyNodeType($arg_1) == 'provider') ) {
 ...
 */
function vb_preprocess_search_results(&$variables) {
...
//  // v1
//  //doesn't work correctly - it create bad links like http://www.voip-weblog.com/search/node/sip%20trunking?page=0%2C0%2C0%2C0%2C0%2C0%2C0%2C1
//  $variables['pager'] = theme('pager', array('tags' => NULL, 'element' => 7));
  ...
  // need prior actions (hack) in function node_search_execute() at node.module
  // v2
  global $vb_num_rows, $vb_limit_rows;
//  dpm('$vb_num_rows =' . $vb_num_rows, '$vb_limit_rows = ' . $vb_limit_rows);
//  dpm($_SESSION['vb_node_search_data']);
//  $page = pager_default_initialize($_SESSION['vb_node_search_data']['vb_num_rows'], $_SESSION['vb_node_search_data']['vb_limit_rows']);
  $page = pager_default_initialize($vb_num_rows, $vb_limit_rows);
 ...
 */
function vb_html_head_alter(&$head_elements) {
...
//  dpm($c_title);
  ...
  if (!empty($head_elements['next']) || !empty($head_elements['prev'])) {
    ...
    if ($current_page > 1) {
      ...
      $current_title = str_replace(' | www.voip-weblog.com', '', $current_title);
...
//      global $user;
//      if ($user->uid == 1) {
//        vb_misc_addMetatag('title', $altered_head_title);
//      }
    }
  }
  //dpm($head_elements);
 ...
 */
function vb_menu_local_tasks(&$variables) {
  $output = '';
...
/*
function vb_preprocess_breadcrumb(&$variables) {
  dpm($variables);
}
*/
 ...
 */
function vb_breadcrumb($variables) {
 ...
 */
function vb_process_page(&$variables) {
...
//        break;
  ...
  */
  elseif ($breadcrumb = vb_misc_getMenuTrail()) {
    //dpm('Page VIA MENU------------');
  ...
  elseif (arg(0) == 'user' && is_int($arg_uid = arg(1))) {
    ...
    $userExtendedData = vb_misc_loadUserExtendedData($arg_uid);
  ...
  // Set a user referer.
  $module_path_misc = drupal_get_path('module', 'vb_misc');
  drupal_add_js( $module_path_misc . '/js/vb_setReferer.js'); 
 ...
 */
function vb_preprocess_node(&$variables) {
 ...
 */
function vb_preprocess_region(&$variables, $hook) {
 ...
 */
function vb_preprocess_block(&$variables, $hook) {
 ...
 *   The name of the template being rendered ("block" in this case.)
 */
function vb_process_block(&$variables, $hook) {
 ...
 */
function vb_preprocess_search_block_form(&$vars) {
 ...
 */
function vb_form_alter(&$form, &$form_state, $form_id) {
...
*/
function vb_preprocess_views_view_archive(&$vars) {
  dpr('xxxxzzzz');
  exit;
  //dpm($vars['view']);
}
...
* Default theme function for all RSS rows.
*/
function vb_preprocess_views_view_row(&$vars) {
  dpm('xxxxxx');
  exit;  
}
...
* Default theme function for all RSS rows.
*/
function vb_preprocess_views_view_row_rss(&$vars) {
  ...
  if (isset($node->body['und'][0]['value'])) {
    $rss_teaser = vb_misc_getArticleTeaserData('all', $node->body['und'][0]['value'], $vars['row']->nid, 400, TRUE);
    ...
    $rss_teaser = preg_replace('|href="/|', 'href="http://www.voip-weblog.com/', $rss_teaser);
...
function vb_preprocess_views_view_unformatted(&$vars) {
//  dpr('yyy');
//exit;
...
function vb_preprocess_views_view(&$vars) {
  //dpm($vars);
  //$vars['view']->rows = 'xxx';
  //$vars['rows'] = 'xxx';
  ...
  /*
  // I set title for preface (at vb_misc_views_pre_render(&$view)) instead of a view itself.
...
//  }
  ...
  if (isset($vars['view']->name) && ($vars['view']->name == 'blog' || $vars['view']->name == 'news') ) {
    $function = 'vb_preprocess_views_view_unformatted'; 
 ...
 * 3. Read carefully, especially within vb_preprocess_html(), there
 ...
 */
function vb_preprocess_html(&$vars) {
  // Load the media queries styles
  // Remember to rename these files to match the names used here - they are
  // in the CSS directory of your subtheme.
//  $media_queries_css = array(
//    'vb.responsive.style.css',
//    'vb.responsive.gpanels.css'
//  );
//  load_subtheme_media_queries($media_queries_css, 'vb');
...
//  );
//  load_subtheme_ie_styles($ie_files, 'vb');
 ...
 * @see comment.tpl.php
 */
function vb_preprocess_comment(&$variables) {
