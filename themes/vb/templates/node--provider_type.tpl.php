...
<?php endif; ?> <!-- if (!$page) -->
      ...
      <?php
        $url = 'http://www.voip-weblog.com' . ($_GET['q'] == 'home' ? '' : $_SERVER['REQUEST_URI']);
        
        $share_title = NULL;
                
        if ($is_front) {
          $share_title = vb_misc_metatag_getFrontTitle();
        }
        ...
        echo '<div class="float share">' . vb_blocks_getSocialiteButtons($url, $share_title) . '</div>';
      ?>
      ...
      */
      ...
      elseif ($node->field_p_types['und'][0]['value'] == 'bu') {
        ...
        if ($view->result) {
          ...
          $results = $view->preview($display_name);
          if ($view->result) {
            module_load_include('inc', 'vb_misc', 'inc/constants');
