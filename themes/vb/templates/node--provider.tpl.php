...
//                         '<div class="site">' , l('Visit ' . $node->field_p_name['und'][0]['value'] /*$content['field_p_name'][0]['#markup']*/, $node->p_data['info']['i_web'], array('external' => TRUE, 'attributes' => array('target' => '_blank'))) , '</div>';
                ...
                ?>  
                    ...
                    <?php /*if (!empty($node->p_data['ereview']['editor_rating_overall']))*/ { ?>
                      ...
                      <?php /*
                        <div class="vb_votes editor">
                          <?php $editor_overall_rating = number_format($node->p_data['ereview']['editor_rating_overall'] * 0.05, 1); ?>
                          <?php echo '<div class="caption"><span><span property="v:reviewer">Editor</span>\'s Overall Rating:</span> <span property="v:rating">' , $editor_overall_rating, '</span>' , '<div class="bottom-clear"></div></div>' , render($node->editor->content['vb_ratings']); ?>
                          <div class="rate-other">
                            <div class="text"><?php echo '<div class="title">' . t('Recommend') . ': </div><div class="data">' . $node->editor->vb_recommend . '</div>'?></div>
                            <?php echo '<div class="voters editor"><div class="count" property="v:count">' . (!empty($node->vb_voters) ? $node->vb_voters : 1) . '</div></div>';?>
                          </div>
                        </div>
                        */
                      ?>
          ...
          <?php echo render($content['metatags']); //vb_misc_renderMetatags_newOrder($content['metatags']);?>
